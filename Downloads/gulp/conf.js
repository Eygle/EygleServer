...
exports.log = {
  ...
  taskStart: (name) => {
    gutil.log(`Starting ${gutil.colors.red('private')} '${gutil.colors.cyan(name)}'...`);
    tasksTimes[name] = Date.now()
  },

  taskFinnished: (name) => {
    gutil.log(`Finished ${gutil.colors.red('private')} '${gutil.colors.cyan(name)}' after ${gutil.colors.magenta(formatTaskDuration(name))}`);
